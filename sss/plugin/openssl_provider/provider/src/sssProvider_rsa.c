    ...
    void *ctx, unsigned char *sig, size_t *siglen, size_t sigsize, const unsigned char *tbs, size_t tbslen)
{
    ...
    if (pRsaCtx->pStoreObjCtx->object.keyId != 0) {
        ...
        if (sig == NULL) {
            *siglen = EVP_PKEY_size(pRsaCtx->pStoreObjCtx->pEVPPkey);
            return 1;
        }
        else {
            int openssl_ret = 0;
...
static int sss_rsa_signature_digest_init(void *ctx, const char *mdname, void *provkey, const OSSL_PARAM params[])
{
    ...
    status = sss_digest_context_init(
        &pRsaCtx->digestCtx, &pRsaCtx->pProvCtx->p_ex_sss_boot_ctx->session, pRsaCtx->sha_algorithm, kMode_SSS_Digest);
    ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
    ...
    if (pRsaCtx->pStoreObjCtx->object.keyId != 0) {
        ...
        if (sig == NULL) {
            *siglen = (((pRsaCtx->pStoreObjCtx->key_len) * 2) + 8);
            ...
            //digest context initialization
            status = sss_digest_context_init(
                &digestCtx, &(pRsaCtx->pProvCtx->p_ex_sss_boot_ctx->session), pRsaCtx->sha_algorithm, kMode_SSS_Digest);
            ENSURE_OR_GO_CLEANUP(status == kStatus_SSS_Success);
