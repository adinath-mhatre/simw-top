...
def main():
    ...
    rsa_ref_key_pair = keys_dir + os.sep + "rsa_ref_prv.pem"

    run("%s genrsa -out %s %d" % (openssl_bin, rsa_key_pair, int(key_size)))
    run("%s rsa -in %s -pubout > %s"%(openssl_bin,rsa_key_pair,rsa_pub_key))
