...
def execute_openssl_cmd(ecc_param_pem, ecc_key_kp, ecc_key_kp_pubonly):
    cmd_str = "\"%s\" ecparam -in \"%s\" -genkey -noout -out \"%s\"" % (openssl_bin,ecc_param_pem, ecc_key_kp)
    run(cmd_str)
    cmd_str = "\"%s\" ec -in \"%s\" -pubout -out \"%s\"" % (openssl_bin, ecc_key_kp, ecc_key_kp_pubonly)
    run(cmd_str)
...
def main():
    ...
    # ECC keys to be stored in SE05X
    # ------------------------------
    ...
    run("\"%s\" ecparam -name %s -out \"%s\"" % (openssl_bin, args.key_type, ecc_param_pem,))
