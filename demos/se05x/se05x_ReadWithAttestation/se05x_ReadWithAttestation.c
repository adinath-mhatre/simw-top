...
static uint8_t count_objPolicies(uint32_t objPolicies);
#endif
#endif
...
#else
    noOfPolices = count_objPolicies(objPolicies);
    if (noOfPolices <= 3) {
        if (Obj_attr.object_origin == kSE05x_Origin_EXTERNAL) {
            if ((LengthTag1Tag2 > 63) && (noOfPolices <= 2)) {
                LOG_W(
                    " When the Secure object has 1 or 2 policies the length of \
                     TAG 41 + TAG 42 MUST NOT be longer than 63 byte ");
                goto exit;
            }
            else if ((LengthTag1Tag2 > 72) && (noOfPolices == 3)) {
                LOG_W(
                    " When the Secure object has 3 policies the length of \
                     TAG 41 + TAG 42 MUST NOT be longer than 72 byte");
                goto exit;
            }
            else if ((LengthTag1Tag2 == 64) && (noOfPolices == 3)) {
                LOG_W(
                    " When the Secure object has 3 policies the length of \
                     TAG 41 + TAG 42 MUST NOT be 64 byte");
                goto exit;
            }
        }
    }
    else {
        LOG_W("Too many policies on object");
        goto exit;
    }
#endif
    status = kStatus_SSS_Success;
...
{
    uint8_t noOfPolices = 0;
    // Iterate through all the polices
    while (objPolicies > 0) {
        // Check if policy is set
        if (objPolicies & 1) {
            noOfPolices++;
        }
        objPolicies = objPolicies >> 1;
    }
    return noOfPolices;
}
#endif //#if SSS_HAVE_SE05X_VER_06_00
