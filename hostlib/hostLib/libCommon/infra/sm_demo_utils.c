...
static char get_mwm_security( WIFISecurity_t security) {
	switch(security) {
		...
		case eWiFiSecurityWPA3:
			return '9';
		default:
			LOG_I("Security mode not supported by the module, security set to eWiFiSecurityWPA2");
			return '4';
	}
}
...
WIFIReturnCode_t network_wifi_connect_ap(void) {
	...
	while (1)
    {
        ...
        if (ret < 0)
        {
        	LOG_E("Failed to get WLAN status.\r\n");
            return eWiFiFailure;
        }
        else if (ret == MWM_INITIALIZED)
        {
        	if (mwm_set_param(MWM_MOD_WLAN, MWM_WLAN_SSID, clientcredentialWIFI_SSID))
        	{
        		LOG_E("Error in setting ssid\r\n");
                return eWiFiFailure;
        	}
        	char security = get_mwm_security(clientcredentialWIFI_SECURITY);
        	if (mwm_set_param(MWM_MOD_WLAN, "security", (char*)&security))
        	{
        		LOG_E("Error in setting security\r\n");
                return eWiFiFailure;
        	}

        	if (mwm_set_param(MWM_MOD_WLAN, MWM_WLAN_PASSPHRASE, clientcredentialWIFI_PASSWORD))
        	{
        		LOG_E("Error in setting passphrase\r\n");
                return eWiFiFailure;
        	}
            /* start wlan */
            ret = mwm_wlan_start();
