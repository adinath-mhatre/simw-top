...
static uint8_t isAlreadyOpen = false;
#endif
...
U16 smComJRCP_Open(void** conn_ctx, const char *hostname_in, unsigned int portnum_in)
{
    ...
    if (isAlreadyOpen) {
        ...
        // TODO disabled check for communication with two simulators
        return SW_OK;
    }
#endif
    ...
    ret =(U32) smCom_Init(&smComJRCP_Transceive, &smComJRCP_TransceiveRaw);
#if SSS_HAVE_APPLET_SE051_UWB
    isAlreadyOpen = true;
#endif
    return (U16)ret;
